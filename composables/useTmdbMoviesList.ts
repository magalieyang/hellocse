...
const fetchMovies = async (page: number = 1) =>{
  loading.value = true;
  try{
    const response = await $fetch<RawMoviesData | RawMoviesErrorData>('/api/moviesList/moviesList', {
      params: { page }
    });
    ...
    if('results' in response){
      ...
      errorOnFetch.value = error instanceof Error ? error.message : 'An error occurred. Please try again.';
  }finally {
    loading.value = false;
  }
...
const fetchMoviesByQuery = async (query: string) => {
  loading.value = true;
  try{
    const response = await $fetch<RawMoviesData | RawMoviesErrorData>('/api/moviesListByQuery/moviesListByQuery', {
      params: { query }
    });
